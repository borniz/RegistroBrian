...
export class Restaurant {
  ...
  async saveEntity(entity: BusinessEntity) { this.isSaving.set(true); try { if (entity.id === undefined) await this.entityService.create(this.table, entity); else await this.entityService.update(this.table, entity); await this.loadEntities(); this.closeEntityModal(); } catch (error) { console.error(error); alert('No se pudo guardar el restaurante.'); } finally { this.isSaving.set(false); } }
  async removeEntity(id: number) { if (!confirm('Desea eliminar el restaurante')) return; try { await this.recordService.deleteRecordsByEntity(id, 'restaurant'); await this.entityService.remove(this.table, id); if (this.selected()?.id === id) { this.selected.set(null); this.records.set([]); } await this.loadEntities(); await this.loadAllRecords(); } catch (error) { console.error('Error eliminando el restaurante y sus registros:', error); } }
  ...
  async saveRecord(data: { description: string; price?: number; record_type: RecordType }) { const entity = this.selected(); if (!entity?.id) return; this.isSaving.set(true); try { const current = this.recordToEdit(); const record = current?.id ? await this.recordService.updateRecord({ ...current, ...data }) : await this.recordService.createRecord({ entity_id: entity.id, entity_type: 'restaurant', ...data }); this.records.update(items => current?.id ? items.map(item => item.id === record.id ? record : item) : [record, ...items]); await this.loadAllRecords(); this.closeRecord(); } catch (error) { console.error(error); alert('No se pudo guardar el registro.'); } finally { this.isSaving.set(false); } }
